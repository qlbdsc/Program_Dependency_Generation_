...
java -jar loggen-0.0.1-SNAPSHOT-jar-with-dependencies-method.jar -i /Users/scduan/Desktop/SCLogger/callgraph_file_selection/1twicetest/project.txt -p $package -c $class -m $method -d /Users/scduan/Desktop/SCLogger/hadoop-trunk >calleemethod.txt
...
prompt=$(cat << EOF 
Instruction:
Insert only one logging statement to the following code using Slf4j. With the format <Line Number>: # <Statement>: #
Java Code：
$funccomment 
...
counter=$((counter + 1))
echo "$prompt" > ./Scloggerprompt/"${counter}_P_${result}.txt"
python3 openai_demo.py --p "$prompt" >> predictionresultSCL.txt
